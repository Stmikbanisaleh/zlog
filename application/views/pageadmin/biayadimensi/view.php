<section class="content">
				...
				<form class="form-horizontal" role="form" id="formEdit">
					<div class="card card-info">
						...
						<div class="card-body">
							...
							<div class="form-group">
								<label>Dimensi Bagi</label>
								<input required type="number" id="e_bagi" name="e_bagi" class="form-control" placeholder="6000">
							</div>
						</div>
						<!-- /.card-body -->
					</div>
	...
	<div class="card">
		<div class="card-header">
			<h3 class="card-title">Biaya Per Dimensi</h3>
		</div>
		<br>
		<br>
...
<script type="text/javascript">
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('administrator/biayadimensi/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				...
				for (i = 0; i < data.length; i++) {
					html += '<tr>' +
						...
						'<td class="text-left">' + data[i].dimensi_bagi + '</td>' +
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/biayadimensi/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
			...
			success: function(data) {
				$('#e_id').val(data[0].id);
				$('#e_harga').val(formatRupiah5(data[0].dimensi_harga, 'Rp. '));
				$('#e_harga_v').val(data[0].dimensi_harga);
				$('#e_bagi').val(data[0].dimensi_bagi);
			}
		});
	});
	...
	if ($("#formEdit").length > 0) {
		$("#formEdit").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: "<?php echo base_url('administrator/biayadimensi/update') ?>",
					type: "POST",
